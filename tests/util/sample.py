...
import tests.util.filesystem as filesystem
...
def start_tidy():
    ...
    with _get_tidy_path() as tidy_dir_path:
        with _get_sample_path() as sample_dir_path:
            filesystem.create_missing_directories_for_dir(sample_dir_path)
            filesystem.create_missing_directories_for_dir(tidy_dir_path)
            i = 0
...
def assert_match(
    ...
    with sample_review_lock:
        with filesystem.temp_file_for_obj(current_str) as cur_path:
...
def _get_sample_path(filename=None):
    ...
    """
    p = os.path.join(
        filesystem.abs_path("../test_docs/sample"), filename or ""
    )
...
def _get_tidy_path(filename=None):
    ...
    """
    p = os.path.join(
        filesystem.abs_path("./test_docs/sample_tidy"), filename or ""
    )
    with sample_path_lock:
        yield p
...
def _clobber_uncontrolled_volatiles(o_str):
    ...
    """
    o_str = re.sub(R"idm\d+", "[IDM]", o_str)
...
def _format_file_name(file_post_str, file_ext_str):
    return "{}{}".format(
        filesystem.get_safe_reversible_path_element(
