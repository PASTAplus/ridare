...
def run_xpath_query(root: lxml.etree._Element, xpath: str) -> list:
    ...
    try:
        ...
        logging.getLogger(__name__).exception(f"Failed to retrieve or parse XPath '{xpath}': {str(e)}")
        return []
...
def build_multi_results(
    ...
    for pid in pids:
        try:
            ...
            logging.getLogger(__name__).exception(f"Failed to retrieve or parse EML for PID {pid}: {str(e)}")
            continue
